...
Formatter for free-format RPGLE in VS Code. Keeps `**FREE` at column 1, applies a configurable base indent, adds block-aware indentation for IF/ELSE/WHEN, loops, SELECT, MONITOR, BEGSR, and procedures, splits semicolon-separated statements so each ends the line (inline blocks included), and corrects both under- and over-indented lines.
...
- Ensures the first line is `**FREE` (uppercase) and removes any duplicate `**FREE` markers later in the file.
...
1. Build or download the VSIX (e.g., `shift6foribmi-0.1.0.vsix`).
